...
import { predictTotalPrice, setPriceLoaded } from '../ReduxSlices/predictPriceSlice'
...
export default function PredictorPrice() {
  ...
  let kWhYAxis = useSelector((state) => state.predicterPrice.kWhTotalPrice)
  let kWhXAxis = [1, 2, 3, 4, 5, 6, 7] // Es gibt nur wöchentliche Vorhersagen

  let periodData = useSelector((state) => state.predicterPrice.dataPrice)
  //Ergebnis des Logins
  ...
  let loaded = useSelector((state) => state.predicterPrice.priceLoaded)
  ...
  return (
    ...
    <View>
      <TouchableOpacity onPress={toggleExpand} style={{ width: ScreenWidth, backgroundColor: '#C3EFBA', margin: 10 }}>
        <Text style={{ fontSize: 17, padding: 10, alignSelf: 'center', fontWeight: 'bold' }}>{isExpanded ? `zum Strompreis schließen` : `zum Strompreis öffnen`}</Text>
      </TouchableOpacity>
      ...
      {isExpanded && (
        <ScrollView horizontal contentContainerStyle={{ flexDirection: 'column' }} style={{ marginTop: 20, paddingRight: 20, paddingLeft: 20, height: '300', width: 'auto', backgroundColor: 'rgba(255, 255, 255, 0.65)', paddingBottom: 20 }}>
          <Text style={{ fontSize: 20, fontWeight: 'bold' }}>Vorhersage des Strompreises</Text>
